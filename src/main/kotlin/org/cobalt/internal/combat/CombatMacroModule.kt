...
object CombatMacroModule : Module("Combat Macro") {
  ...
  fun onTick(@Suppress("UNUSED_PARAMETER") event: TickEvent.Start) {
    ...
    if (spiderWarpStage >= 0) {
      val levelW = mc.level ?: run { cancelSpiderWarp(); return }
      handleSpiderCombatWarp(player, levelW)
      return
    }
    ...
    if (inAttackRange || inKeepDistanceZone) {
    ...
    } else if (activeCombatMode == 0 && shouldUseSpiderInstantTransmission(player) && hasLos && dist <= SPIDER_IT_RANGE) {
      // IT weapon in range: stop pathing, face spider, attack — the hit triggers the teleport.
      if (startedPath && nativeActive()) nativeStop()
      startedPath = false
      lastTargetPos = null
      stuckRepathCount = 0
      RotationExecutor.rotateTo(aimRotation, rotationStrategy)
      if (!suppressAttack) attemptAttack(player, target, 0)
      ...
      // Try etherwarp to reach distant spider before falling through to the native pathfinder.
      if (shouldUseSpiderCombatEtherwarp() && !isSlayerBossEntity(target) &&
          dist >= spiderEtherwarpMinDist.value && level.gameTime >= spiderWarpCooldownUntilTick) {
        if (tryStartSpiderCombatWarp(player, level, target)) return
      }
  ...
  private fun onKillCandidateConfirmed(resolvedName: String) {
    if (killCandidateWasSlayerMob) {
      ...
      // Allow immediate re-path to the next target without waiting for the throttle interval.
      lastPathStartTick = (mc.level?.gameTime ?: 0L) - MIN_PATH_START_INTERVAL_TICKS
  ...
  // ── Spider combat etherwarp ────────────────────────────────────────────────

  private fun shouldUseSpiderCombatEtherwarp(): Boolean =
    cryptZombieSlayer.value && slayerType.value == 2 && spiderEtherwarpEnabled.value &&
      !isSpiderBossHatchlingsPhaseActive() && !slayerBossActive

  private fun tryStartSpiderCombatWarp(player: Player, level: ClientLevel, target: LivingEntity): Boolean {
    val etherwarpSlot = EtherwarpLogic.findEtherwarpHotbarSlot()
    if (etherwarpSlot < 0) return false
    // Aim at the spider's lower torso — the etherwarp raytrace should hit the floor below it.
    val aimPt = net.minecraft.world.phys.Vec3(target.x, target.y + 0.5, target.z)
    val currentSlot = player.inventory.selectedSlot
    if (currentSlot != etherwarpSlot) {
      InventoryUtils.holdHotbarSlot(etherwarpSlot)
      spiderWarpRestoreSlot = currentSlot
    } else {
      spiderWarpRestoreSlot = -1
    }
    spiderWarpAimPoint = aimPt
    spiderWarpStage = 0
    spiderWarpStageTick = level.gameTime
    if (startedPath && nativeActive()) nativeStop()
    startedPath = false
    lastTargetPos = null
    return true
  ...
  private fun handleSpiderCombatWarp(player: Player, level: ClientLevel) {
    val aimPt = spiderWarpAimPoint
    if (aimPt == null || !shouldUseSpiderCombatEtherwarp()) {
      cancelSpiderWarp()
      return
    }
    val stageDuration = level.gameTime - spiderWarpStageTick
    val rot = AngleUtils.getRotation(aimPt)
    when (spiderWarpStage) {
      0 -> { // Rotate to aim point
        RotationExecutor.rotateTo(org.cobalt.api.util.helper.Rotation(rot.yaw, rot.pitch), rotationStrategy)
        val yawErr = abs(AngleUtils.getRotationDelta(player.yRot, rot.yaw))
        val pitchErr = abs(rot.pitch - player.xRot)
        if ((yawErr <= SPIDER_WARP_AIM_TOLERANCE && pitchErr <= SPIDER_WARP_AIM_TOLERANCE) ||
            stageDuration >= SPIDER_WARP_STAGE_TIMEOUT_TICKS) {
          mc.options.keyShift?.setDown(true)
          spiderWarpStage = 1
          spiderWarpStageTick = level.gameTime
        }
      }
      1 -> { // Hold shift, verify warp is aimed at a valid block, then click
        RotationExecutor.rotateTo(org.cobalt.api.util.helper.Rotation(rot.yaw, rot.pitch), rotationStrategy)
        mc.options.keyShift?.setDown(true)
        val warpResult = EtherwarpLogic.getEtherwarpResultSneaking()
        if (warpResult.succeeded) {
          mc.options.keyUse?.setDown(true)
          spiderWarpStage = 2
          spiderWarpStageTick = level.gameTime
        } else if (stageDuration >= SPIDER_WARP_STAGE_TIMEOUT_TICKS) {
          // Can't find a warp target — give up and let the pathfinder handle it.
          spiderWarpCooldownUntilTick = level.gameTime + SPIDER_WARP_COOLDOWN_TICKS
          cancelSpiderWarp()
        }
      }
      2 -> { // Release keys and restore slot
        mc.options.keyUse?.setDown(false)
        mc.options.keyShift?.setDown(false)
        if (spiderWarpRestoreSlot in 0..8) {
          InventoryUtils.holdHotbarSlot(spiderWarpRestoreSlot)
        }
        spiderWarpCooldownUntilTick = level.gameTime + SPIDER_WARP_COOLDOWN_TICKS
        spiderWarpStage = -1
        spiderWarpAimPoint = null
        spiderWarpRestoreSlot = -1
      }
    }
  }

  private fun cancelSpiderWarp() {
    if (spiderWarpStage >= 0) {
      mc.options.keyUse?.setDown(false)
      mc.options.keyShift?.setDown(false)
      if (spiderWarpRestoreSlot in 0..8) {
    ...
    spiderWarpAimPoint = null
    spiderWarpRestoreSlot = -1
    spiderWarpStageTick = -1L
  }

  // ── Spider Instant Transmission ───────────────────────────────────────────

  private fun shouldUseSpiderInstantTransmission(player: Player): Boolean {
    if (!cryptZombieSlayer.value || slayerType.value != 2 || !spiderITEnabled.value) return false
    if (slayerBossActive) return false
    return hasInstantTransmissionWeapon(player)
  }

  private fun hasInstantTransmissionWeapon(player: Player): Boolean {
    val stack = player.mainHandItem
    if (stack.isEmpty) return false
    return stack.getLoreLines().any { line ->
      line.string.lowercase().contains("instant transmission")
    }
  }
  ...
  private const val SPIDER_WARP_AIM_TOLERANCE = 5f
  private const val SPIDER_WARP_STAGE_TIMEOUT_TICKS = 12L
  private const val SPIDER_WARP_COOLDOWN_TICKS = 15L
  private const val SPIDER_IT_RANGE = 15.0
